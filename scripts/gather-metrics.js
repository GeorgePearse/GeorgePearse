 ...
 */
async function getRepoCommitCount(repo, days = 365) {
  if (!GITHUB_TOKEN) {
 ...
 */
async function estimateRepositoryLOC(repo) {
  if (!GITHUB_TOKEN) {
 ...
 */
function generateHistoricalMetrics(currentMetrics) {
  const metrics = [];
  const now = new Date();
  ...
  for (let i = 12; i >= 0; i--) {
    ...
    metrics.push({
      ...
      totalCommits: Math.round(
        currentMetrics.commits * (trendFactor + noise)
      ),
      totalStars: Math.round(
        currentMetrics.stars * (trendFactor + noise * 0.5)
      ),
...
async function main() {
  ...
  const cachedReposPath = path.join(__dirname, "../src/data/cached-repos.json");
  const cachedReposData = JSON.parse(
    fs.readFileSync(cachedReposPath, "utf-8")
  );
  ...
  const outputPath = path.join(__dirname, "../src/data/repos-metrics.json");
